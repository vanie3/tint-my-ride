...
//        var_dump($wp_query->post->ID);
	...
	private function registerAjaxCallbacks() {
		//TODO: Rewrite
		add_action('wp_ajax_motopress_ce_posts_grid_load_more', array($this, 'motopressPostsGridLoadMore'));
...
//                            'terms' => $custom_tax_terms,
            ...
            if (!$is_ajax) {
                if ($pagination == 'true' && $load_more_btn == 'false') {
                    ...
                    if ($paged < $pagesCount) {
	                    ...
	                    //TODO: Rewrite (define general global variable + nonces)
                        wp_localize_script('mp-posts-grid', 'mpPostsGrid', array(
                                ...
                                <script type="text/javascript">
                                    function loadMorePostsFor<?php echo $gridId; ?>() {
                                        var data = {
	                                        ...
	                                        nonce: '<?php echo wp_create_nonce('wp_ajax_motopress_ce_posts_grid_load_more'); ?>',
                                            <?php
                                            ...
                                            ?>
                                            ajax_paged: 1 // Change it in motopressLoadMorePosts
                                        };
                                        motopressLoadMorePosts('<?php echo $gridId; ?>', data);
                                    }
                                </script>
                            <?php
    ...
    public function motopressPostsGridLoadMore() {
        ...
        $fields = array('query_type', 'post_type', 'columns', 'category', 'tag',  'template', 'posts_gap', 'show_featured_image', 'image_size', 'image_custom_size', 'title_tag', 'show_date_comments', 'show_content', 'short_content_length', 'read_more_text', 'ajax_paged');
